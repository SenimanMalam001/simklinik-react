...
class ItemKeluarCreate extends React.Component {
  constructor() {
    super()
    this.state = {
      jumlah: '',
  ...
  handleSubmit = (event) => {
    ...
    if (this.validate()) {
      ...
      const headers = {
        token,
        otoritas: 'create_item_keluar'
      }

      axios.post('/item-keluar',{jumlah, produk, keterangan},{ headers }).then((res) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(ItemKeluarCreate)
